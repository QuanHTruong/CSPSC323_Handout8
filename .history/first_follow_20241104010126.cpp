            ...
            for (const auto &[nonTerminal, rules] : productions) {
                for (const string &rule : rules) {
                    for (size_t i = 0; i < rule.length(); i++) {
                        char symbol = rule[i];
                        if (isupper(symbol)) { // Only process non-terminals
                            set<char> tempFollow;

                            // Check what's following the non-terminal in the production
                            for (size_t j = i + 1; j < rule.length(); j++) {
                                char nextSymbol = rule[j];

                                if (isupper(nextSymbol)) { // Next is a non-terminal
                                    // Add FIRST(nextSymbol) except for ε
                                    for (char ch : firstSets[nextSymbol]) {
                                        if (ch != 'ε') {
                                            tempFollow.insert(ch);
                                        }
                                    }
                                    // If FIRST(nextSymbol) contains ε, continue to add FOLLOW of nonTerminal
                                    if (firstSets[nextSymbol].count('ε') > 0) {
                                        // Look ahead for the next symbols in the rule
                                        if (j + 1 < rule.length()) {
                                            continue; // Continue looking at the next symbols
                                        } else {
                                            // Add FOLLOW(nonTerminal) to FOLLOW(symbol) if at the end of the rule
                                            for (char ch : followSets[nonTerminal]) {
                                                if (followSets[symbol].insert(ch).second) {
                                                    updated = true; // If there was an addition
                                                }
                                            }
                                        }
                                    } else {
                                        break; // Stop processing after adding FIRST(nextSymbol)
                                    }
                                } else { // Next is a terminal
                                    tempFollow.insert(nextSymbol);
                                    break; // Stop looking after a terminal
                                }
                            }

                            // Add collected FOLLOW symbols
